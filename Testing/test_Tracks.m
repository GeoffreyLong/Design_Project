function [ output_args ] = test_Tracks( resultFileBase, nFrames, rect, truth )
...
%   [id frameNo bbox]
  ...
  for i = 1:nTracks
    ...
    for j = 1:size(curId,1)
      valid = 0;
      curRect = curId(j,:);
      curTruth = truth(truth(:,1)==curRect(1),2:5);
      for k = 1:size(curTruth,1)
        if (bboxOverlapRatio(curTruth, curRect, 'ratioType', 'min') >= 0.5)
          valid = 1;
        end
      end
      if (valid)
