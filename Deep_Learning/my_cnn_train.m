function [ net, info ] = my_cnn_train(net, imdb, varargin)
% An adaptation of examples/cnn_train.m
%   net: the structure of the convolutional neural network
%       Can visualize with vl_simplenn_display(net, 'inputSize', [64 64 1 50])
%   imdb: A set of images
...
% How far back we want the error to be propogated
...
%NOTE idk why this doesn't work
...
for epoch=start+1:opts.numEpochs
  ...
  if numGpus <= 1
    [net,stats.train,prof] = process_epoch(opts, epoch, train, learningRate, imdb, net) ;
    [~,stats.val] = process_epoch(opts, epoch, val, 0, imdb, net) ;
    if opts.profile
      profile('viewer') ;
      keyboard ;
    end
    ...
    spmd(numGpus)
      [net_, stats_train_,prof_] = process_epoch(opts, epoch, train, learningRate, imdb, net) ;
      [~, stats_val_] = process_epoch(opts, epoch, val, 0, imdb, net_) ;
    end
    net = net_{1} ;
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,prof] = process_epoch(opts, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
